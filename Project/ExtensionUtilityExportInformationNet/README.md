...
- **project/main/ExtensionUtilityExportInformationNet.csproj** — the extension itself (net8.0-windows);
- **viewer/main/ProjectInfoViewer.csproj** — the web viewer (net8.0), published automatically as a part of the extension build.

Before building, check the **CAMPath** property in **project/main/ExtensionUtilityExportInformationNet.csproj** — it must point to the **Bin64** folder of the installed CAM system, because all CAMAPI references are resolved from there.

There are two equivalent ways to build (both require .NET 8 SDK):
