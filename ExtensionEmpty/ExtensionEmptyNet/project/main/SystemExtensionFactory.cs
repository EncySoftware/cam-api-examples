...
public static class SystemExtensionFactory
{
    ...
    public static ExtensionNetProject.ApiComObject<T> CreateExtension<T>(string extensionTypeId, IExtensionInfo? info)
    {
        using var instanceInfo = new ExtensionNetProject.ApiComObject<IExtensionInstanceInfo>(info?.InstanceInfo);
        using var extensionManager = new ExtensionNetProject.ApiComObject<IExtensionManager>(instanceInfo.Instance.ExtensionManager);
        ...
        return new ExtensionNetProject.ApiComObject<T>(typedExtension);
    }
    ...
    /// <typeparam name="T">Type, which extension should be casted to</typeparam>
    /// <returns>Instance of extension</returns>
    public static ExtensionNetProject.ApiComObject<T> GetSingletonExtension<T>(string extensionTypeId, IExtensionInfo? info)
    {
        using var instanceInfo = new ExtensionNetProject.ApiComObject<IExtensionInstanceInfo>(info?.InstanceInfo);
        using var extensionManager = new ExtensionNetProject.ApiComObject<IExtensionManager>(instanceInfo.Instance.ExtensionManager);
