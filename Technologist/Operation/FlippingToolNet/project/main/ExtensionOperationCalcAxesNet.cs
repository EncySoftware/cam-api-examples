...
/// </summary>
internal class ExtensionOperationCalcAxesNet : IExtension, IExtensionUtility
{
    /// <inheritdoc />
    public IExtensionInfo? Info { get; set; }

    private static InvToolParameters ShowTransformationDialogInv(List<FlipInfo> flips)
    {
        InvToolParameters result = null;

        var thread = new Thread(() =>
        {
            var window = new InvTextInputWindow(flips);
            if (window.ShowDialog() == true)
                result = window.SettingsInv;
        });
    ...
    public void Run(IExtensionUtilityContext context, out TResultStatus resultStatus)
    {
        ...
        try
        {
            ...
            var flipLines = new List<FlipInfo>();
            ...
            {
                var flipsCount = machineConfiguration.FlipsCount;
                for (var i = 0; i < flipsCount; i++)
                {
                    var flipId = machineConfiguration.FlipId[i];
                    var flipCaption = machineConfiguration.FlipCaption[i];
                    var flipEnabled = machineConfiguration.FlipEnabled[i];
                    
                    flipLines.Add(
                        new FlipInfo { Id = flipId, Caption = flipCaption, Enabled = flipEnabled }
                    );
                }
            });
            ...
            var settings = ShowTransformationDialogInv(flipLines);
            if (settings == null)
                return;
            ...
            machineConfigurationCom.Invoke(machineConfiguration =>
            {
                ...
                for (var i = 0; i < flipsCount; i++)
                {
                    var flipInfo = settings.Flips[i];
                    machineConfiguration.FlipEnabled[i] = flipInfo.Enabled;
                }
            });

            // tool below
            position.n.X = settings.X;
            position.n.Y = settings.Y;
            position.n.Z = settings.Z;
